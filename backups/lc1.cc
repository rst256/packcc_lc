...
statement <- 
	'do' _ e:statements _ 'end'  { printf("block: %s: %d %d\n", $0, $0s, $0e); }
	/ ( assign / define_var_stat ) _ ';' { printf("statement: %s: %d %d\n", $0, $0s, $0e); }
	/ if_stat _
...
if_stat <- 
	'if' _ expression _ 'then' statements _ 'end'
...
define_var_stat <- 
	te:type_expr _ ':' ~ { ERROR("expected `:` %s\n", $0); } _ ( assign / < IDENT > )
